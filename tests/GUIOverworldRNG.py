...
class Application(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.generate = tk.Button(self, text="Generate", command=self.generate,width=20)
        ...
        tk.Checkbutton(self, text="Shiny Charm", variable=self.shiny_charm_var).grid(column=0,row=2,columnspan=2)
        tk.Checkbutton(self, text="Mark Charm", variable=self.mark_charm_var).grid(column=2,row=2,columnspan=2)
        tk.Checkbutton(self, text="Is Static", variable=self.is_static_var).grid(column=0,row=3,columnspan=2)
        tk.Checkbutton(self, text="Is Legendary", variable=self.is_legendary_var).grid(column=2,row=3,columnspan=2)
        tk.Checkbutton(self, text="Weather Active", variable=self.weather_active_var).grid(column=0,row=4,columnspan=2)
        tk.Checkbutton(self, text="Is Fishing", variable=self.is_fishing_var).grid(column=2,row=4,columnspan=2)
        tk.Checkbutton(self, text="Rand Held Item", variable=self.diff_held_item_var).grid(column=0,row=5,columnspan=2)
        tk.Checkbutton(self, text="Double Mark Gen", variable=self.double_mark_gen_var).grid(column=2,row=5,columnspan=2)
        tk.Checkbutton(self, text="Is Shiny Locked", variable=self.is_shiny_locked_var).grid(column=0,row=6,columnspan=4)
        tk.Label(self, text="Level:").grid(column=0,row=10)
        ...
        tk.Label(self,text="Filter").grid(column=4,row=1,columnspan=3)
        tk.Label(self,text="Shininess").grid(column=4,row=2,columnspan=1)
        self.shiny_filter = ttk.Combobox(self, values=["Any","Star","Square","Star/Square"])
        self.shiny_filter.grid(column=5,row=2,columnspan=2)
        self.shiny_filter.set("Any")
        self.has_mark_filter = tk.IntVar()
        tk.Checkbutton(self, text="Has Mark", variable=self.has_mark_filter).grid(column=4,row=3,columnspan=3)
        self.rare_mark_filter = tk.IntVar()
        tk.Checkbutton(self, text="Has Rare Mark", variable=self.rare_mark_filter).grid(column=4,row=4,columnspan=3)
        self.specific_mark_filter = tk.IntVar()
        tk.Checkbutton(self, text="Specific Mark", variable=self.specific_mark_filter).grid(column=4,row=5)
        self.specific_mark = ttk.Combobox(self,values=["Rare","Uncommon","Weather","Time","Fishing"]+OverworldRNG.personality_marks)
        self.specific_mark.grid(column=5,row=5,columnspan=2)
        self.slot_filter = tk.IntVar()
        tk.Checkbutton(self, text="Slot", variable=self.slot_filter).grid(column=4,row=6)
        self.min_slot_var = tk.Spinbox(self, from_= 0, to = 99, width = 5)
        self.max_slot_var = tk.Spinbox(self, from_= 0, to = 99, width = 5)
        self.min_slot_var.grid(column=5,row=6)
        self.max_slot_var.grid(column=6,row=6)
        self.advances_track = tk.Label(self,text="0")
        self.advances_track.grid(column=4,row=10)
        tk.Label(self,text="+").grid(column=5,row=10)
        self.max_advance_var = tk.Spinbox(self, value=99999, from_= 0, to = 99999999, width = 20)
        self.max_advance_var.grid(column=6,row=10)
        self.autofill = tk.Button(self,text="Autofill",command=self.autofill_info)
        self.autofill.grid(column=8,row=1)
